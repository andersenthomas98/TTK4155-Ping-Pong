...
void MENU_playGame(void) {
	...
	timer_1division256Init(); //start timer to count score
	while (GAME_OVER != 1) {
		TIM8_WriteTCNT0(0);
		...
		//only send new values over CAN
		if ((abs(currentSliderValue - lastSliderValue) > 1) || (abs(currentJoystickValue - lastJoystickValue) > 5) || (currentButtonValue != lastButtonValue)){
